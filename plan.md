...
- 配置：`.env` 解析（`python-dotenv`，以 `override=True` 加载）
- LLM 通信：官方 Python SDK —— `anthropic`（`AsyncAnthropic`）、`openai`（`AsyncOpenAI`），均原生支持
  async 流式（SDK 内部已处理 SSE）

## 架构概览（分层）
1. 入口层 `Arkcode.cli` —— 加载配置、打印 banner、启动 Textual App。
2. 配置层 `Arkcode.config` —— 读取并校验项目根目录 `.env`，给出有序 providers 列表。
3. LLM 协议层 `Arkcode.llm` —— 定义协议无关的 `Provider` Protocol 与统一消息/流式事件类型；
   anthropic、openai 两个适配器各自封装官方 SDK、统一吐出文本增量（思考增量内部丢弃）。
4. 会话层 `Arkcode.conversation` —— 进程内维护多轮历史，提供完整上下文。
5. 提示词/资源 `Arkcode.prompt` —— 内置 system prompt 与启动 banner（ASCII 猫）。
6. 终端层 `Arkcode.tui` —— Textual App，含状态机（选择/空闲/流式）、输入框、对话区、
...
def load(path: str) -> Config: ...     # dotenv override=True 加载 + 校验
...
### 模块 `Arkcode.config`
职责：读取并校验项目根目录 `.env`，产出有序 providers 列表。
对外接口：`load(path) -> Config`；`Config.providers`。
加载规则：`load_dotenv(path, override=True)` 后读取 `ARKCODE_PROVIDERS`；按逗号切分并保留
         声明顺序，每个原始名称去除首尾空格后作为显示名，再转为大写拼接
         `ARKCODE_<NAME>_{PROTOCOL,MODEL,BASE_URL,API_KEY,THINKING}`。
校验规则：provider 列表非空；名称仅允许字母、数字、下划线且大小写归一后不可重复；
         每项 `protocol` / `api_key` / `model` 非空；`protocol ∈ {"anthropic", "openai"}`；
         `thinking` 缺省为 `false` 且仅接受 `true` / `false`；空 `base_url` 转为 `None`。
         任一不满足 → 抛出 `ConfigError` 携带可读变量名，不包含任何密钥值。
安全规则：`ProviderConfig.api_key` 使用 `field(repr=False)`，避免对象表示意外回显密钥。
依赖：`python-dotenv`、标准库 `pathlib` / `os` / `re`。
...
对外接口：`ArkCodeApp(providers: list[ProviderConfig])`；`App.run()` 或 `await App.run_async()`。
内部职责：
...
### 模块 `Arkcode.cli`（入口）
职责：装配与启动。
流程：`config.load(...)` → `prompt.render_banner(version, cwd)` 打印 →
`ArkCodeApp(cfg.providers).run()`，使用 Textual 默认的标准全屏/独立屏幕缓冲区，不传
`inline` / `inline_no_clear`。
...
├── README.md
├── .env                            — 本地运行配置（git 忽略）
├── .env.example                    — 脱敏的多 provider 配置模板
├── src/
│   └── Arkcode/
│       ├── __init__.py
│       ├── __main__.py             — 允许 `python -m Arkcode`
...
```
说明：
- 依赖版本预期：`textual`、`rich`、`anthropic`、`openai`、`python-dotenv`。在 `pyproject.toml` 中
  以 `dependencies = [...]` 声明，锁文件用 `uv.lock`（推荐 `uv`）或 `pip-compile` 生成的
  `requirements.txt`。
- `tui/` 拆 4 个文件按职责切分；若实现时过碎可合并，不影响接口。
- `.env` 含真实密钥，应在 `.gitignore` 忽略；提交一份不含真实密钥的 `.env.example`。
  旧 YAML 配置不再读取；若本地存在则不主动删除。
- `pyproject.toml` 里通过 `[project.scripts] Arkcode = "Arkcode.cli:main"` 暴露 CLI 入口；
  装好后既可 `Arkcode` 也可 `python -m Arkcode`。

## 技术决策

| 决策点 | 选择 | 理由 |
|--------|------|------|
| 语言 | Python 3.12+ | 项目既定（Arkcode python 线）；3.12 的 typing/`asyncio.TaskGroup` 等更舒服 |
...
| 渲染模型 | 标准全屏 Textual + `RichLog.write(...)` 追加 | 独立屏幕缓冲区可可靠使用完整终端尺寸，避免 inline 模式在 shell 光标下方空间不足时覆盖、截断顶部会话；运行期间可在 `RichLog` 内滚动 |
| 输入区对齐 | `#prompt` 与 `#input` 使用相同内容起始行，不给提示符额外顶部 padding | 保证 80×24 等窄高终端中 `❯` 与输入文字同一基线 |
...
| 配置 | 根目录 `.env` + `python-dotenv`（`override=True`） | 支持有序多 provider；`.env` 覆盖系统变量；N5 密钥安全 |
