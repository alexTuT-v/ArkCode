...
from Arkcode.application import ApplicationRuntime, cli
from Arkcode.application import bootstrap as bootstrap_module
...
def test_main_registers_mcp_tools_and_closes_manager(
    ...
    def fake_new_app(runtime: ApplicationRuntime) -> FakeApp:
        observed_names.extend(runtime.tools.get_deferred_tool_names())
        observed_names.extend(
            item.name
            for item in runtime.tools.definitions()
            if runtime.tools.is_discovered(item.name)
        )
        observed_statuses.append(runtime.mcp_status)
        return FakeApp()

    write_valid_env(tmp_path / ".env")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(
        bootstrap_module.mcp_client, "load_config", lambda root: Config()
    )
    monkeypatch.setattr(bootstrap_module.mcp_client, "new_manager", fake_new_manager)
