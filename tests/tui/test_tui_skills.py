...
from Arkcode.tools.skill_tools.install_skill import Params as InstallParams
...
async def test_multi_provider_selection_completes_skill_assembly(
    ...
    second = ProviderConfig(
        name="second", protocol="openai", api_key="secret", model="second-model"
    )
    app = ArkCodeApp(
        [config(), second],
        "1.0",
...
@pytest.mark.asyncio
async def test_clear_failure_preserves_conversation_journal_and_active_skills(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    ...
    async with app.run_test() as pilot:
        ...
        old_journal = app.journal
        monkeypatch.setattr(
            "Arkcode.application.session.new_session_context",
        ...
        assert app.journal is old_journal
...
async def test_install_tool_callback_hot_registers_command_and_catalog(
    ...
    async with app.run_test() as pilot:
        ...
        result = await app.install_skill_tool.execute(
            InstallParams(url="https://skills.sh/acme/repo/remote")
        )
